...
def get_top_likes_list():
    return {'documents': Document.objects.annotate(num_likes=Count('liked_by')).order_by('-num_likes')[:5]}
...
def get_document_recommendations(document):
    ...
    return {'documents': set(document_recommendations.annotate(num_likes=Count('liked_by')).order_by('-num_likes')[:5])}
...
def get_document_in_category(category):
    return {'documents': Document.objects.filter(topic__id__contains=category.id)}


@register.inclusion_tag('document/top_likes.html')
def get_top_document_in_category(category):
    return {'documents': Document.objects.filter(topic__id__contains=category.id).annotate(
        num_likes=Count('liked_by')).order_by('-num_likes')[:5]}
